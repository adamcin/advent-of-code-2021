...
5,5 -> 8,2";
    ...
    fn day06read(filename: &str) -> FishPool {
        ...
        let mut cohorts: [FishCohort; 7] = [FishCohort { size: 0, maturity: 0 }; 7];
        for cohort_age in 0..7 {
            let aged: Vec<usize> = fishes.iter().filter(|&age| *age == cohort_age).cloned().collect();
            cohorts[(cohort_age + 1) % 7] = FishCohort { maturity: 0, size: aged.len() as u128 };
        }
        return FishPool { day: 0, matures: cohorts, noobs: Vec::new() };
    }
    ...
    fn day06part1() {
        ...
        for day in 0..80 {
            pool.tick();
            if day == 0 {
                assert_eq!(300, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 1 {
                assert_eq!(430, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 2 {
                assert_eq!(474, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 3 {
                assert_eq!(510, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 4 {
                assert_eq!(561, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 5 {
                assert_eq!(600, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 6 {
                assert_eq!(600, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 7 {
                assert_eq!(600, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 8 {
                assert_eq!(730, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 14 {
                assert_eq!(1200, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 20 {
                assert_eq!(2196, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 40 {
                assert_eq!(12211, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            } else if day == 79 {
                assert_eq!(365862, pool.fish_count(), "expect number of fishes after day {}", day + 1);
            }
        }
